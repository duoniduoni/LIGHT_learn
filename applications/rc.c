...
/*
 *  2022.9.15补充解释：
 *  1. Call_RadioContrl间隔2ms被调用一次。
 *  2. 处理的数据来自driver_pwm_in.c文件中的Rc_Pwm_In[8]数组（8通道），数组中的值为1000~2000。
 *  3. Call_RadioContrl函数对driver_pwm_in中的数据进一步处理：
 *      a. 先将1000~2000的值变换成 -500 ~ 500范围内，变换是先比例变换然后横移，变换结果存在CH[8]中。
 *      b. 对每个通道的变化量做限制：
 *          I. 先得到变化量（本次的控制数据 - 上次记录的数据）。
 *          II. 生成一个系数3.14 * 20 * 间隔时间（单位是秒），因为2ms调用一次，这个间隔很小，所以说这个系数应该不大。
 *          III. 最终结果是上次记录的控制量 + (这次控制量 - 上次控制量) * 系数。如果变化量大于100，再 * 0.5进一步限制。
 *          IV. 最终结果存储在CH_filter[8]数组中。其他地方直接使用这个数组。
 */
 ...
 *  
 *  确定定高模式：
 *  1. 通道AUX1 在-500 ~ -200之间（即开关处于低位），不定高。
 *  2. 通道AUX1 在-200 ~ 200之间，气压定高。
 *  3. 其余的情况，依赖超声波模块是否可用；可用则 超声波定高，否则气压定高。
 *
 *  小总结：
 *      飞控程序使用了5个通道分别是油门(THR)、和rol/yaw/pit以及AUX1。
 */
